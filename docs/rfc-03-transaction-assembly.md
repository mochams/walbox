...
- No accounting of streamed-transaction memory against the bounded delivery queue's
  ...
  configured bound. This is a known, deliberate v0.1 limitation (see README's
  Limitations).
...
with no duplicated buffering/emission logic to keep in sync. The cost is a small amount of
...
- Two streamed transactions' chunks interleaved on the wire (`StreamStart(A) ...
  ...
  their own changes, in their own order, with no cross-contamination between buffers.
