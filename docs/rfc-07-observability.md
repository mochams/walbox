...
point in the existing run loop where that event already happens — no restructuring
...
The obvious approach — reuse the client's existing "last written LSN" feedback field
...
- A callback that raises is caught and logged, and does not stop replication from
  continuing — verified by confirming the run loop is still alive and processing
  afterward.
