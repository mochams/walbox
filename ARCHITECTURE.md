...
research into replication-transport internals established that libpq itself
already owns CopyData/CopyDone envelope framing in both directions. `protocol.py`
never implements that framing; it only decodes/encodes the messages *inside* an
...
| During checkpoint save | The previous durable checkpoint remains valid (`PostgresCheckpointStore`'s transactional rollback leaving the prior committed row intact); transaction redelivered. |
